...
import yep.greenFire.greenfirebackend.user.seller.domain.entity.Seller;
...
public interface SellerRepository extends JpaRepository<Seller, Long> {
    ...
    @Query("SELECT new yep.greenFire.greenfirebackend.user.seller.dto.response.StoreListResponse" +
            ...
            "WHERE m.memberCode = :memberCode AND s.applyStatus = yep.greenFire.greenfirebackend.user.seller.domain.type.ApplyStatus.APPLY")
    List<StoreListResponse> findByMemberCode(@Param("memberCode") Long memberCode);

    // 특정 스토어 프로필 조회
    @Query("SELECT new yep.greenFire.greenfirebackend.user.seller.dto.response.StoreProfileResponse" +
